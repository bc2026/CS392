        ...
        do
        {
          ...
          if (result == -1)
          {
            if (errno == EINTR)
            {
              ...
              interrupted = 0;
              printf("\n");
              break; // If interrupted by SIGINT, just break from the loop
            }
            else
            {
              fprintf(stderr, "Error: wait() failed. %s.\n", strerror(errno));
              exit(EXIT_FAILURE); // For other errors, exit or handle accordingly
            }
          }

        } while (!WIFEXITED(status) && !WIFSIGNALED(status)); // Repeat if waitpid was interrupted
      }
    }
